...
class Cube:
    ...
    def remplir_nb(self, iPiece, nb): # insere une piece transformée
        for x, y, z in positions[iPiece]:
            self.coord[x][y][z] = nb
...
selon_coordonnee = list(arrangement())
...
if __name__ == "__main__":
    ...
    resultat = principal()
    resultat_piece = sorted([positions[i] for i in resultat])

    
    for i, iPiece in enumerate(resultat_piece):
        print('La piece p' + str(i+1), 'a pour coordonnées', sorted(iPiece))

    nouveau_cube = Cube([[[True]*5 for i in range(5)] for i in range(5)])
    for i, iPiece in enumerate(resultat):
        nouveau_cube.remplir_nb(iPiece, i)

    print('\n\nAfficher coupe cube\n')


    for x in nouveau_cube.coord:
        for y in x:
            print(*y)
        print()
        print()
